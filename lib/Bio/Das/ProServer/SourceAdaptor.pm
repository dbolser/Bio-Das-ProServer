...
our $VERSION  = do { my ($v) = (q$Revision: 708 $ =~ /\d+/mxsg); $v; };
...
sub das_entry_points {
  ...
  if ($rows_end < 1 || $rows_start > $total || $rows_start > $rows_end) {
    croak _error(400, 402, "Requested rows are outside the valid range (1-$total)");
  }
...
sub das_alignment {
  ...
  if (!scalar @queries) {
    croak _error(400, 402, q(The 'query' parameter is required.));
  }
  my @subjects = grep { $_ } @{ $opts->{'subjects'} };
  if (scalar @subjects && $rows) {
    croak _error(400, 402, q(The 'subject' and 'rows' parameters may not both be present in the same request.));
  }
  if ($rows && $rows !~ /^\d+-\d+/mxs) {
    croak _error(400, 402, q(The 'rows' parameter must be of format: 'START-END'));
  }
  if ($cols && $cols !~ /^\d+-\d+/mxs) {
    croak _error(400, 402, q(The 'cols' parameter must be of format: 'START-END'));
  }
...
sub das_structure {
  ...
  for my $chain (@{ $chains }) {
    $chain =~ m/^[A-Za-z0-9]$/mxs || croak _error(400, 402, "Requested chain ($chain) must be a single alphanumeric character");
  }
  for my $model (@{ $models }) {
    $model =~ m/^\d+$/mxs || croak _error(400, 402, "Requested model ($model) must be an integer");
  }
