...
our $VERSION  = do { my ($v) = (q$Revision: 710 $ =~ /\d+/mxsg); $v; };
...
sub _load_registry {
  ...
  if (!$self->config->{'skip_registry'}) {
    Bio::EnsEMBL::Registry->load_registry_from_db(
      -host => $self->config->{'reg_host'} || 'ensembldb.ensembl.org',
      -user => $self->config->{'reg_user'} || 'anonymous',
      -pass => $self->config->{'reg_pass'},
      -port => $self->config->{'reg_port'},
      -verbose => $self->{'debug'} );
  }
