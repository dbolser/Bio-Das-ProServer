...
our $VERSION  = do { my ($v) = (q$Revision: 704 $ =~ /\d+/mxsg); $v; };
...
sub query {
  ...
  for (split /\s(?:AND|&&)\s/mxsi, $query) {
    ...
    if ($cmp eq q(=) || $cmp eq q(==) || $cmp eq q(eq)) {
      ...
      push @queries, sub { $_[$field] =~ /^$value$/msi }; ## no critic(RegularExpressions::RequireExtendedFormatting)
    } elsif ($cmp eq '>=') {
...
sub _query_fh {
  ...
  LINE: while(my $line = <$fh>) {
    chomp $line;
    $line || next;
    $comment && $line =~ m/$comment/ms && next; ## no critic(RegularExpressions::RequireExtendedFormatting)
    $i++;
    my @parts = split /$sep/ms, $line; ## no critic(RegularExpressions::RequireExtendedFormatting)
